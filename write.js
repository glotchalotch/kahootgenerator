...
function makeEnds() {
    ...
    for(i = 1; i < questionNum + 1; i++) {
        ...
        zip.file(`data/kahoot/functions/questions/q${i}end.mcfunction`, `function kahoot:questions/qendg\nschedule function kahoot:${funcStr} 100`);
    }
}

function makePres() {
    for(i = 2; i < questionNum + 1; i++) {
        zip.file(`data/kahoot/functions/questions/q${i}pre.mcfunction`, `title @a times 10 70 20\ntitle @a title "Question ${i}"\ntitle @a subtitle "Begins in 10 seconds"\nschedule function kahoot:questions/q${i} 200`);
    }
}
...
function makeAnsTest() {
    ...
    zip.file(`data/kahoot/functions/answertest.mcfunction`, `${startStr}`);
}

async function zipUp() {
    zip.generateAsync({ type: "blob" }).then((data) => {
        saveAs(data, "kahoot.zip");
    });
}